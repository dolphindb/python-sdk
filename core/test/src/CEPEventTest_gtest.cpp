...
public:
    ...
    static void SetUpTestSuite()
    {
        ...
        try
        {
            ConstantSP res = connsp->run("1+1");
        }
        catch(const std::exception& e)
        {
            connsp->connect(hostName, port, "admin", "123456");
        }
    }
...
TEST_F(CEPEventTest, test_EventSchema_null)
{
    ...
    string re = "";
    DBConnection& tmpconn = *connsp.get();
    try
    {
        EventSender *sender = new EventSender(tmpconn, "", eventSchemas, eventTimeKeys, commonKeys);
    }
    catch (exception &ex)
    {
        re = ex.what();
...
TEST_F(CEPEventTest, test_EventType_special_character)
{
    ...
    vector<string> commonKeys = {};
    DBConnection& tmpconn = *connsp.get();
    EventSender *sender = new EventSender(tmpconn, "inputTable", eventSchemas, eventTimeKeys, commonKeys);
...
TEST_F(CEPEventTest, test_eventSchema_exception)
{
    ...
    DBConnection& tmpconn = *connsp.get();
    EXPECT_ANY_THROW(EventSender(tmpconn, "", eventSchemas1, {}, {}));
    EXPECT_ANY_THROW(EventSender(tmpconn, "", eventSchemas2, {}, {}));
...
TEST_F(CEPEventTest, test_EventSender_connect_not_connect)
{
    ...
    try{
        EventSender* sender = new EventSender(tmp_conn, "inputTable", eventSchemas, eventTimeKeys, commonKeys);
    }catch(exception& ex){
        re = ex.what();
    }
...
TEST_F(CEPEventTest, test_EventSender_commonKeys_cols_not_match)
{
    ...
    EXPECT_EQ("Schema mismatch: Output table contains 3 cols, expected 4 cols.", re);
    delete schema;
}
...
TEST_F(CEPEventTest, test_EventSender_conn_asynchronousTask_true) //TODO
{
    ...
    vector<string> eventTimeKeys = {"time"};
    vector<string> commonKeys = {};
    DBConnection tmp_connsp(false, true);
...
TEST_F(CEPEventTest, test_EventSender_conn_ssl_true)
{
    ...
    vector<string> eventTimeKeys = {"time"};
    vector<string> commonKeys = {};
    DBConnection tmp_connsp(true, false);
    tmp_connsp.connect(hostName, port, "admin", "123456");
    EventSender* sender = new EventSender(tmp_connsp, "inputTable", eventSchemas, eventTimeKeys, commonKeys);
...
TEST_F(CEPEventTest, test_EventSender_conn_compress_true)
{
    ...
    vector<string> eventTimeKeys = {"time"};
    vector<string> commonKeys = {};
    DBConnection tmp_connsp(false, false, 7200, true);
    tmp_connsp.connect(hostName, port, "admin", "123456");
    EventSender* sender = new EventSender(tmp_connsp, "inputTable", eventSchemas, eventTimeKeys, commonKeys);
...
TEST_F(CEPEventTest, test_EventSender_conn_not_admin)
{
    ...
    vector<string> eventTimeKeys = {"time"};
    vector<string> commonKeys = {};
    DBConnection tmp_connsp(false, false, 7200, false);
...
TEST_F(CEPEventTest, test_eventTable_not_exist)
{
    ...
    DBConnection& tmpconn = *connsp.get();
    try{
        EventSender* sender = new EventSender(tmpconn, "notExistTable", eventSchemas, eventTimeKeys, commonKeys);
    }catch(const exception& ex){
        re = ex.what();
    }
...
TEST_F(CEPEventTest, test_EventSender_sendEvent_eventType_not_exist)
{
    ...
    EXPECT_EQ("Failed to serialize, with error Unknown eventType market1", re);
    delete sender, schema;
}
...
TEST_F(CEPEventTest, test_EventSender_sendEvent_eventType_null)
{
    ...
    EXPECT_EQ("Failed to serialize, with error Unknown eventType ", re);
    delete sender, schema;
}
...
TEST_F(CEPEventTest, test_EventSender_sendEvent_attributes_column_not_match)
{
    ...
    EXPECT_EQ("Failed to serialize, with error The number of event values does not match market", re);
    delete sender, schema;
}
...
TEST_F(CEPEventTest, test_EventSender_sendEvent_attributes_type_not_match)
{
    ...
    EXPECT_EQ("Failed to serialize, with error Expected type for the field time of market : TIME, but now it is INT", re);
    delete sender, schema;
}
...
TEST_P(CEPEventTest_EventSender_alltypes, test_EventSender_alltype)
{
    ...
    DBConnection& tmpconn = *connsp.get();
    EventSender* sender = new EventSender(tmpconn, "fromApi", eventSchemas, eventTimeKeys, commonKeys);
    int rows = 1000;
