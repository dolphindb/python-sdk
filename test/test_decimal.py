...
def get_ex_row0(quantileVal):
    return [decimal.Decimal("1").quantize(quantileVal),
...
class TestDecimal:
    ...
    def test_decimal32_list_download(self):
        self.conn.run("""
                    a = decimal32(-10.1 -1 NULL 0.1 4.1 5.1, 3)
                   """)
        a = self.conn.run("a")
        b = np.array([decimal.Decimal("-10.100"),
    ...
    def test_decimal64_list_download(self):
        self.conn.run("""
                    a = decimal64(-10.1 -1 NULL 0.1 4.1 5.1, 3)
                   """)
        a = self.conn.run("a")
        b = np.array([decimal.Decimal("-10.100"),
                      decimal.Decimal("-1.000"),
    ...
    def test_decimal128_list_download(self):
        self.conn.run("""
                    a = decimal128("-10.1" "-1" NULL "0.1" "4.1" "5.1", 36)
                   """)
        a = self.conn.run("a")
        b = np.array([decimal.Decimal("-10.100000000000000000000000000000000000"),
                      decimal.Decimal("-1.000000000000000000000000000000000000"),
                      None,
    ...
    def test_decimal_with_other_type_list_upload(self):
        ...
        self.conn.upload({"a": a})
        b = self.conn.run(
            "eqObj(a, (NULL, decimal64(-10.1, 3), 1.1, NULL, 123, `aa, NULL))")
        assert b
    ...
    def test_decimal_np_array_upload(self):
        a = np.array([None, decimal.Decimal("-10.100"),
                      decimal.Decimal("-1.000"),
                      None,
                      decimal.Decimal("0.100"),
                      decimal.Decimal("4.100"),
        ...
        self.conn.upload({"a": a})
        b = self.conn.run(
            "eqObj(a, decimal64(NULL -10.1 -1 NULL 0.1 4.1 5.1 NULL, 3))")
        assert b

        a = np.array([None, decimal.Decimal("-10.1234567899999999999"),
                      decimal.Decimal("-1.1234567899999999999"),
                      None,
                      decimal.Decimal("0.1234567899999999999"),
                      decimal.Decimal("4.1234567899999999999"),
                      decimal.Decimal("5.1234567899999999999"),
                      None])
        self.conn.upload({"a": a})
        b = self.conn.run(
    ...
    def test_decimial_dict_key_upload(self):
        a = {decimal.Decimal("-10.100"): decimal.Decimal("-10.100"),
             "b": decimal.Decimal("NaN"), "c": decimal.Decimal("2.0")}
        with pytest.raises(Exception):
            self.conn.upload({"a": a})

    def test_decimal_set_upload(self):
        a = {decimal.Decimal("-10.100"),
             decimal.Decimal("NaN"), decimal.Decimal("2.0")}
        with pytest.raises(Exception):
            self.conn.upload({"a": a})
    ...
    def test_multithreadTableWriterTest_decimal(self):
        ...
        writer = ddb.MultithreadedTableWriter(
            HOST, PORT, "admin", "123456", "", "decimal_t", False, False, [], 100, 0.1, 1, "")
