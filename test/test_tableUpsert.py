...
class TesttableUpsert:
    ...
    def test_tableUpsert_keyedTable_key_exist_ignoreNull_False(self, pickle, compress):
        ...
        sym = np.repeat(['AAPL', 'GOOG', 'MSFT', 'IBM', 'YHOO'], 2, axis=0)
        date = np.array(['2012-01-01', 'NaT', '1965-07-25', 'NaT', '2020-12-23', '1970-01-01', 'NaT', 'NaT', 'NaT', '2009-08-05'], dtype="datetime64")
        qty = np.arange(1, 11)
        qty1 = np.arange(2, 12)
        data = pd.DataFrame({'sym': sym, 'date': date, 'qty': qty})
    ...
    def test_tableUpsert_DFS_key_exist_ignoreNull_False(self, pickle, compress):
        ...
        """
        conn.run(script)
        upsert=ddb.tableUpsert(dbPath="dfs://tableUpsert_test",tableName="pt",ddbSession=conn,ignoreNull=False,keyColNames=["qty"])
        sym = np.repeat(['AAPL', 'GOOG', 'MSFT', 'IBM', 'YHOO'], 2, axis=0)
        date = np.array(['2012-01-01', 'NaT', '1965-07-25', 'NaT', '2020-12-23', '1970-01-01', 'NaT', 'NaT', 'NaT', '2009-08-05'], dtype="datetime64")
        qty1 = np.arange(2, 12)
        data1 = pd.DataFrame({'sym': sym, 'date': date, 'qty': qty1})
    ...
    def test_tableUpsert_DFS_key_exist_ignoreNull_True(self, pickle, compress):
        ...
        """
        conn.run(script)
        upsert=ddb.tableUpsert(dbPath="dfs://tableUpsert_test",tableName="pt",ddbSession=conn,ignoreNull=True,keyColNames=["qty"])
        sym = np.repeat(['AAPL', 'GOOG', 'MSFT', 'IBM', 'YHOO'], 2, axis=0)
        date = np.array(['2012-01-01', 'NaT', '1965-07-25', 'NaT', '2020-12-23', '1970-01-01', 'NaT', 'NaT', 'NaT', '2009-08-05'], dtype="datetime64")
        qty1 = np.arange(2, 12)
        data1 = pd.DataFrame({'sym': sym, 'date': date, 'qty': qty1})
    ...
    def test_tableUpsert_dfs_table_alltype(self, pickle, compress):
        ...
        ''')
        upsert=ddb.tableUpsert(dbPath="dfs://tableUpsert_test",tableName="pt",ddbSession=conn,ignoreNull=True,keyColNames=["int"])
        df = pd.DataFrame({
    ...
    def test_tableUpsert_dfs_table_alltype_arrayvector(self, pickle, compress):
        ...
        ''')
        upsert=ddb.tableUpsert(dbPath="dfs://tableUpsert_test",tableName="pt",ddbSession=conn,ignoreNull=True,keyColNames=["id"])
        df = pd.DataFrame({
    ...
    @pytest.mark.parametrize('pickle', [False, False], ids=["EnPickle", "UnPickle"])
    @pytest.mark.parametrize('compress', [True, False], ids=["EnCompress", "UnCompress"])
    def test_tableUpsert_dfs_table_arrayvector_not_match_1(self, pickle, compress):
        conn = ddb.session(HOST, PORT, USER, PASSWD, enablePickle=pickle, compress=compress)
        conn.run('''
        dbPath = "dfs://tableUpsert_test"
        if(existsDatabase(dbPath))
    ...
    @pytest.mark.parametrize('pickle', [False, False], ids=["EnPickle", "UnPickle"])
    @pytest.mark.parametrize('compress', [True, False], ids=["EnCompress", "UnCompress"])
    def test_tableUpsert_dfs_table_arrayvector_not_match_2(self, pickle, compress):
        conn = ddb.session(HOST, PORT, USER, PASSWD, enablePickle=pickle, compress=compress)
        conn.run('''
        dbPath = "dfs://tableUpsert_test"
        if(existsDatabase(dbPath))
    ...
    @pytest.mark.parametrize('pickle', [False, False], ids=["EnPickle", "UnPickle"])
    @pytest.mark.parametrize('compress', [True, False], ids=["EnCompress", "UnCompress"])
    def test_tableUpsert_dfs_table_arrayvector_not_match_3(self, pickle, compress):
        conn = ddb.session(HOST, PORT, USER, PASSWD, enablePickle=pickle, compress=compress)
        conn.run('''
        dbPath = "dfs://tableUpsert_test"
        if(existsDatabase(dbPath))
    ...
    @pytest.mark.parametrize('pickle', [False, False], ids=["EnPickle", "UnPickle"])
    @pytest.mark.parametrize('compress', [True, False], ids=["EnCompress", "UnCompress"])
    def test_tableUpsert_dfs_table_arrayvector_not_match_4(self, pickle, compress):
        conn = ddb.session(HOST, PORT, USER, PASSWD, enablePickle=pickle, compress=compress)
        conn.run('''
        dbPath = "dfs://tableUpsert_test"
        if(existsDatabase(dbPath))
    ...
    def test_TableUpserter_dfs_table_alltype(self, pickle, compress):
        ...
        pt = db.createPartitionedTable(t, `pt, `int,,`int)
        ''')
        upsert=ddb.TableUpserter(dbPath="dfs://tableUpsert_test",tableName="pt",ddbSession=conn,ignoreNull=True,keyColNames=["int"])
        df = pd.DataFrame({
            'bool': np.array([True, False], dtype=np.bool8),
            'char': np.array([1, -1], dtype=np.int8),
            'short': np.array([-10, 1000], dtype=np.int16),
    ...
    @pytest.mark.parametrize('_order', ['F', 'C'], ids=["F_ORDER", "C_ORDER"])
    @pytest.mark.parametrize('_python_list', [True, False], ids=["PYTHON_LIST", "NUMPY_ARRAY"])
    def test_TableUpsert_upsert_dataframe_arrayVector_with_numpy_order(self,_compress,_order,_python_list):
        conn1 = ddb.session(HOST,PORT,USER,PASSWD,compress=_compress)
        data = []
        for i in range(10):
    ...
    @pytest.mark.parametrize('_order', ['F', 'C'], ids=["F_ORDER", "C_ORDER"])
    @pytest.mark.parametrize('_python_list', [True, False], ids=["PYTHON_LIST", "NUMPY_ARRAY"])
    def test_TableUpsert_upsert_null_dataframe_arrayVector_with_numpy_order(self,_compress,_order,_python_list):
        conn1 = ddb.session(HOST,PORT,USER,PASSWD,compress=_compress)
        data = []
        origin_nulls = [[None],[np.nan],[pd.NaT]]
