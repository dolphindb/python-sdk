...
class TestDocs:
    ...
    def test_BasicOperation_AsynchronousWrite_BTW(self):
        s = ddb.Session()
        s.connect(HOST, PORT, USER, PASSWD)
    ...
    def test_AdvancedOperation_ObjectorientedOperationOnDatabase_Table_3(self):
        ...
        # assert_array_equal(res2.columns, ['ticker', 'date', 'bid'])
    ...
    def test_AdvancedOperation_ObjectorientedOperationOnDatabase_Table_4(self):
        ...
        # res2 = trade.select('count(ask)').groupby(
        #     ['vol']).having('count(ask)>1').toDF()
        # ex2 = s.run(
        #     "select count(ask) from loadTable('dfs://valuedb', `trade) group by vol having count(ask)>1")
    ...
    def test_AdvancedOperation_ObjectorientedOperationOnDatabase_Table_5(self):
        ...
        for i in range(len(res2)):
            ...
            # print('-----------------------------')
            # if i == 2:
            #     res2[i] = [d.strftime('%Y.%m.%d')
            #                for d in res2[i].astype('datetime64[D]').tolist()]
