...
class TestMultithreadTableWriter:
    ...
    def insert_all_data_type(self, writer, id):
        ...
        col_decimal32 = np.array([decimal.Decimal("NaN"), decimal.Decimal('0.000000'), decimal.Decimal('-1'),
                                  decimal.Decimal('2.000000'), decimal.Decimal('3.123123'), decimal.Decimal('4.0')], dtype='object')
        col_decimal64 = np.array([decimal.Decimal("NaN"), decimal.Decimal('0.0000000000000000'), decimal.Decimal('-1.0000000000000000'),
                                  decimal.Decimal('2.0000000000000000'), decimal.Decimal('3.123123123123123'), decimal.Decimal('4.0')], dtype='object')
        col_decimal128 = np.array([decimal.Decimal("NaN"), decimal.Decimal('0.000000000000000000000000'), decimal.Decimal('-1.000000000000000000000000'),
                                   decimal.Decimal('2.000000'), decimal.Decimal('3.123123123123123123123123'), decimal.Decimal('4.0')], dtype='object')
        for i in range(6):
            res = writer.insert(col_bool[i], col_char[i], col_short[i], col_int[i], col_long[i],
                                ...
                                col_double[i], col_string[i], col_uuid[i], col_int128[i], col_ipaddr[i], col_decimal32[i],col_decimal64[i],col_decimal128[i])
            if res.hasError():
    ...
    def insert_all_type_compress(self, writer, id):
        ...
        for i in range(6):
            res = writer.insert(col_bool[i], col_char[i], col_short[i], col_int[i], col_long[i], col_date[i], col_month[i], col_time[i], col_second[i], col_minute[i],
                                col_datehour[i], col_datetime[i], col_timestamp[i], col_nanotime[i], col_nanotimestamp[i], col_float[i], col_double[i], col_string[i], col_arrayVector[i])
            if res.hasError():
                print("error code ", res.errorCode, res.errorInfo)
                break
    ...
    def test_multithreadTableWriterTest_error_mode(self):
        with pytest.raises(RuntimeError,match='Unsupported mtw mode TEST'):
            writer = ddb.MultithreadedTableWriter(
                HOST, PORT, USER, PASSWD, self.dbname, "pdatetest", False, False, [], 100, 0.1, 10, "date",mode="test")
    ...
    def test_multithreadTableWriterTest_error_compressMethods(self):
        with pytest.raises(RuntimeError,match='Unsupported compression method TEST'):
            writer = ddb.MultithreadedTableWriter(
                HOST, PORT, USER, PASSWD, self.dbname, "pdatetest", False, False, [], 100, 0.1, 10, "date",compressMethods=["test"])
    ...
    def test_multithreadTableWriterTest_function_getUnwrittenData_dfs_2(self):
        ...
        self.conn.run(script)
        writer = ddb.MultithreadedTableWriter(
            HOST, PORT, USER, PASSWD, self.dbname, "tmp", batchSize=1000, threadCount=10, partitionCol="date",mode="APPEND")
    ...
    """
        ...
        col_decimal32 = np.array([None, decimal.Decimal('0.000000'), decimal.Decimal('-1.000000'),
                                  decimal.Decimal('2.000000'), decimal.Decimal('3.123123'), decimal.Decimal('4.000000')], dtype='object')
        col_decimal64 = np.array([None, decimal.Decimal('0.0000000000000000'), decimal.Decimal('-1.0000000000000000'),
                                  decimal.Decimal('2.0000000000000000'), decimal.Decimal('3.123123123123123'), decimal.Decimal('4.0')], dtype='object')
        col_decimal128 = np.array([None, decimal.Decimal('0.000000000000000000000000'), decimal.Decimal('-1.000000000000000000000000'),
                                   decimal.Decimal('2.000000'), decimal.Decimal('3.123123123123123123123123'), decimal.Decimal('4.0')], dtype='object')
        ex = pd.DataFrame({
            "cbool": col_bool,
            "cchar": col_char,
    ...
    """
        ...
        col_decimal32 = np.array([None, decimal.Decimal('0.00'), decimal.Decimal('-1.00'),
                                  decimal.Decimal('2.00'), decimal.Decimal('3.12'), decimal.Decimal('4.00')], dtype='object')
    ...
    """
        ...
        for i in range(10):
            threads.append(threading.Thread(
                target=self.insert_TSDB_Huge, args=(writer, i,)))

        self.conn.run("delete from loadTable('dfs://valuedb_DFS_TSDB','pt')")
        first = self.conn.run(
            "select count(*) from loadTable('dfs://valuedb_DFS_TSDB','pt')")
        for t in threads:
            t.daemon = True
            t.start()
        for t in threads:
        ...
        print(last["count"][0]-first["count"][0])

        re = self.conn.run(
            "select * from loadTable('dfs://valuedb_DFS_TSDB','pt')")
    ...
    """
        ...
        compress = ["LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4",
                    "LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4", "LZ4"]
        writer = ddb.MultithreadedTableWriter(
    ...
    def test_multithreadTableWriter_max_length_string_in_memory_table(self):
        ...
        assert self.conn.run(f'strlen({tbname}[`str][0])==262143')
        self.conn.run(f'undef `{tbname},SHARED')
        del mtw
    ...
    def test_multithreadTableWriter_max_length_symbol_in_memory_table(self):
        ...
        tbname = 't_' + random_string(5)
        self.conn.run(f"""
        share table(100:0,[`str],[SYMBOL]) as {tbname}
        """)
        mtw=ddb.MultithreadedTableWriter(HOST,PORT,USER,PASSWD,'',tbname,threadCount=1)
        mtw.insert(data)
        mtw.waitForThreadCompletion()
    ...
    def test_multithreadTableWriter_max_length_blob_in_memory_table(self):
        ...
        tbname = 't_' + random_string(5)
        self.conn.run(f"""
        share table(100:0,[`str],[BLOB]) as {tbname}
        """)
        mtw=ddb.MultithreadedTableWriter(HOST,PORT,USER,PASSWD,'',tbname,threadCount=1)
        mtw.insert(data)
        mtw.waitForThreadCompletion()
        status=mtw.getStatus()
        assert not status.hasError()
        assert self.conn.run(f'strlen({tbname}[`str][0])==262144')
        self.conn.run(f'undef `{tbname},SHARED')
        del mtw
    ...
    def test_multithreadTableWriter_max_length_string_stream_table(self):
        ...
        tbname = 't_' + random_string(5)
        self.conn.run(f"""
        share streamTable(100:0,[`str],[STRING]) as {tbname}
        """)
        mtw=ddb.MultithreadedTableWriter(HOST,PORT,USER,PASSWD,'',tbname,threadCount=1)
        mtw.insert(data)
        mtw.waitForThreadCompletion()
    ...
    def test_multithreadTableWriter_max_length_symbol_stream_table(self):
        ...
        tbname = 't_' + random_string(5)
        self.conn.run(f"""
        share streamTable(100:0,[`str],[SYMBOL]) as {tbname}
        """)
        mtw=ddb.MultithreadedTableWriter(HOST,PORT,USER,PASSWD,'',tbname,threadCount=1)
        mtw.insert(data)
        mtw.waitForThreadCompletion()
    ...
    def test_multithreadTableWriter_max_length_blob_stream_table(self):
        ...
        tbname = 't_' + random_string(5)
        self.conn.run(f"""
        share streamTable(100:0,[`str],[BLOB]) as {tbname}
        """)
        mtw=ddb.MultithreadedTableWriter(HOST,PORT,USER,PASSWD,'',tbname,threadCount=1)
        mtw.insert(data)
        mtw.waitForThreadCompletion()
    ...
    def test_multithreadTableWriter_max_length_blob_dfs_table(self):
        ...
        status=mtw.getStatus()
        assert not status.hasError()
        assert self.conn.run(f'select strlen(id) from tb')['strlen_id'][0]==4*64*1024
        del mtw
