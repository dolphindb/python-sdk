...
def loadPlugin():
    ...
    res = False
    try:
        if conn.run(f"""login("admin","123456");go;loadPlugin('{ARROW_DIR}')"""):
            print("server加载Arrow插件成功，测试开始")
            res = True
        else:
            res = False
        ...
        if "already in use" in str(e):
            print("server已加载Arrow插件，测试开始")
            res = True
        else:
            print("server加载Arrow插件失败，原因是：", err)
            res = False
    conn.close()
    return res
...
class TestSession:
    ...
    def test_session_protocol_error(self):
        with pytest.raises(RuntimeError,match="Protocol 4 is not supported. "):
            conn=ddb.Session(HOST, PORT, USER, PASSWD,protocol=4)
        with pytest.raises(RuntimeError,match="The Arrow protocol does not support compression."):
            conn=ddb.Session(HOST, PORT, USER, PASSWD,protocol=keys.PROTOCOL_ARROW,compress=True)
    ...
    # 跑该测试时，pytest需要加上-s参数，避免stderr信息无法被捕获
    @pytest.mark.SESSION
    ...
    def test_session_init_enableChunkGranularityConfig(self, _compress, _pickle):
        ...
        db.createPartitionedTable(
            table=t, tableName=tablename, partitionColumns="col3", sortColumns="col2")
    ...
    def test_session_runFile(self, _compress, _pickle):
        ...
        # db = conn1.database(dbPath="dfs://test")
    ...
    # TODO: 开启Arrow和不开启时，上传下载table的性能对比
    @pytest.mark.ARROW
    def test_session_download_table_enableArrow_with_supportedTypes(self):
        ...
        df = pd.DataFrame({
            ...
            'cdate': np.array(['1970-01-21 13:30:10.008', '1970-01-01 00:00:00.000000000', 'nat'], dtype='datetime64[D]'),
            ...
            'cuuid': np.array(["5d212a78-cc48-e3b1-4235-b4d91473ee87", "5d212a78-cc48-e3b1-4235-b4d91473ee99", ""], dtype='object'),
            'cint128': np.array(["e1671797c52e15f763380b45e841ec32", "e1671797c52e15f763380b45e841ec79", ""], dtype='object'),
            ...
            'cdecimal64': np.array([decimal.Decimal("-1").quantize(decimal.Decimal("0.0000000000")), None, decimal.Decimal(
                "0").quantize(decimal.Decimal("0.0000000000"))], dtype='object'),
            'cdatehour': np.array(['1970-01-01 13:30:10.008', '2022-10-03 05:15:33.335245880', 'nat'], dtype='datetime64[h]'),
        })
        ...
        ex_tab = pyarrow.table({
            ...
            'cint': np.array([-10, 1000, 0], dtype=np.int32),
            'clong': np.array([-10, 1000, 0], dtype=np.int64),
            'ctime': pyarrow.array(np.array(['1970-01-01 13:30:10.008', '1970-01-01 05:15:33.335245880', 'nat'], dtype='datetime64[ms]'), type=pyarrow.time32('ms')),
            'cdate': pyarrow.array([datetime.datetime(1970, 1, 21), datetime.datetime(1970, 1, 1), None], type=pyarrow.date32()),
            'cmonth': pyarrow.array([datetime.datetime(2020, 1, 1), datetime.datetime(1970, 12, 1), None], type=pyarrow.date32()),
            'cminute': pyarrow.array([datetime.time(hour=13, minute=30, second=00), datetime.time(hour=5, minute=15, second=00), None], type=pyarrow.time32('s')),
            'csecond': pyarrow.array(np.array(['1970-01-01 13:30:10.008', '1970-01-01 05:15:33.335245880', 'nat'], dtype='datetime64[s]'), type=pyarrow.time32('s')),
            'cdatetime': pyarrow.array(np.array(['1970-01-01 13:30:10.008', '2022-10-03 05:15:33.335245880', 'nat'], dtype='datetime64[s]'), type=pyarrow.timestamp('s')),
            'ctimestamp': pyarrow.array(np.array(['2012-06-13 13:30:10.008', '2001-04-22 15:18:29.118325481', 'nat'], dtype='datetime64[ms]'), type=pyarrow.timestamp('ms')),
            'cnanotimestamp': pyarrow.array(np.array(['1970-01-01 13:30:10.008007006', '1970-01-01 21:08:02.008007006', 'nat'], dtype='datetime64[ns]'), type=pyarrow.timestamp('ns')),
            ...
            'csymbol': pyarrow.DictionaryArray.from_arrays(pyarrow.array([1, 2, None], type=pyarrow.int32()), pyarrow.array(['', 'sym1', 'sym2'])),
            ...
            'cuuid': pyarrow.array([b']!*x\xccH\xe3\xb1B5\xb4\xd9\x14s\xee\x87', b']!*x\xccH\xe3\xb1B5\xb4\xd9\x14s\xee\x99', None], type=pyarrow.binary(16)),
            'cint128': pyarrow.array([b'\xe1g\x17\x97\xc5.\x15\xf7c8\x0bE\xe8A\xec2',b'\xe1g\x17\x97\xc5.\x15\xf7c8\x0bE\xe8A\xecy', None], type=pyarrow.binary(16)),
            ...
            'cdatehour': pyarrow.array(np.array(['1970-01-01 13:00:00', '2022-10-03 05:00:00', 'nat'], dtype='datetime64[s]'), type=pyarrow.timestamp('s')),
        })
        # print('\n手工构造： ', ex_tab['cdecimal32'], '--------------------------\n')
        # print('下载的： ', tab['cdecimal32'])
        assert (tab.equals(ex_tab))

    @pytest.mark.ARROW
    @pytest.mark.parametrize('_coltype, _patype', [('bool', None), ('char', pyarrow.int8()),
                                                   ('int', pyarrow.int32()), ('short', pyarrow.int16()), ('long', pyarrow.int64())], ids=['bool', 'char', 'int', 'short', 'long'])
    def test_session_download_table_enableArrow_with_arrayVector_1(self, _coltype, _patype):
        assert loadPlugin()
        conn1 = ddb.session(HOST, PORT, protocol=keys.PROTOCOL_ARROW)
        t = conn1.run(f"""
                ind = 1 3 4;
    ...
    @pytest.mark.ARROW
    @pytest.mark.parametrize('_coltype, _patype', [('float', pyarrow.float32()), ('double', pyarrow.float64())], ids=['float', 'double'])
    def test_session_download_table_enableArrow_with_arrayVector_2(self, _coltype, _patype):
        assert loadPlugin()
        conn1 = ddb.session(HOST, PORT, protocol=keys.PROTOCOL_ARROW)
        t = conn1.run(f"""
                ind = 1 3 4;
    ...
    @pytest.mark.parametrize('_coltype, _patype', [('time', pyarrow.time32('ms')), ('minute', pyarrow.time32('s')),
                                                   ('second', pyarrow.time32('s'))], ids=['time', 'minute', 'second'])
    def test_session_download_table_enableArrow_with_arrayVector_3(self, _coltype, _patype):
        assert loadPlugin()
        conn1 = ddb.session(HOST, PORT, protocol=keys.PROTOCOL_ARROW)
        t = conn1.run(f"""
                ind = 1 3;
    ...
    @pytest.mark.ARROW
    @pytest.mark.parametrize('_coltype, _patype', [('date', pyarrow.date32()), ('datetime', pyarrow.timestamp('s')), ('datehour', pyarrow.timestamp('s')),
                                                   ('timestamp', pyarrow.timestamp('ms')), ('nanotime', pyarrow.time64('ns')), ('nanotimestamp', pyarrow.timestamp('ns'))],
                             ids=['date', 'datetime', 'datehour', 'timestamp', 'nanotime', 'nanotimestamp'])
    def test_session_download_table_enableArrow_with_arrayVector_4(self, _coltype, _patype):
        assert loadPlugin()
        conn1 = ddb.session(HOST, PORT, protocol=keys.PROTOCOL_ARROW)
        t = conn1.run(f"""
                ind = 1 3;
    ...
    @pytest.mark.ARROW
    def test_session_download_table_enableArrow_with_arrayVector_5(self):
        assert loadPlugin()
        conn1 = ddb.session(HOST, PORT, protocol=keys.PROTOCOL_ARROW)
        t = conn1.run("""
                ind = 1 3;
        ...
        ex_t = pyarrow.table({
            ...
            'col5': pyarrow.array([[None], [b']!*x\xccH\xe3\xb1B5\xb4\xd9\x14s\xee\x87', b']!*x\xccH\xe3\xb1B5\xb4\xd9\x14s\xee\x87']], type=pyarrow.list_(pyarrow.binary(16))),
            'col6': pyarrow.array([[None], [b'\xe1g\x17\x97\xc5.\x15\xf7c8\x0bE\xe8A\xec2', b'\xe1g\x17\x97\xc5.\x15\xf7c8\x0bE\xe8A\xec2']], type=pyarrow.list_(pyarrow.binary(16))),
        })
        assert t.equals(ex_t)

    @pytest.mark.ARROW
    def test_session_download_table_enableArrow_with_arrayVector_month(self):
        assert loadPlugin()
        conn1 = ddb.session(HOST, PORT, protocol=keys.PROTOCOL_ARROW)
        scripts = """
        t1 = table(month([0, 1, 1970*12+1, 1970*12+100, NULL]) as cmonth)
        """
        conn1.run(scripts)
        re: pyarrow.Table = conn1.run("t1")

        conn2 = ddb.session(HOST, PORT, "admin", "123456")
        conn2.run(scripts)
    ...
    def test_session_download_table_enableArrow_with_unsupported_types(self):
        ...
        with pytest.raises(RuntimeError, match="Arrow Not support Decimal ArrayVector"):
            conn1.run(
                "index=1 3 5;val=decimal32(1 2 3 4 5,2);av=arrayVector(index, val);table(av as col1)")

        with pytest.raises(RuntimeError, match="Arrow Not support Decimal ArrayVector"):
            conn1.run(
                "index=1 3 5;val=decimal64(1 2 3 4 5,10);av=arrayVector(index, val);table(av as col1)")
    ...
    def test_session_upload_table_enableArrow(self, compress):
        if compress:
            with pytest.raises(RuntimeError, match="The Arrow protocol does not support compression."):
                conn1 = ddb.session(
                    HOST, PORT, protocol=keys.PROTOCOL_ARROW, compress=compress)
        else:
            conn1 = ddb.session(
                HOST, PORT, protocol=keys.PROTOCOL_ARROW, compress=compress)
            tab = conn1.run(
                "tab = table(1 2 3 as c1, `a`b`c as c2, date(1 5 9) as c3);tab")
            c1 = tab['c1'].to_pylist()
            conn1.upload({'ex_c1': c1})
            assert conn1.run('`tab in objs().name')

    @pytest.mark.ARROW
    def test_func_with_enableArrow(self):
        assert loadPlugin()
        conn1 = ddb.session(
            HOST, PORT, protocol=keys.PROTOCOL_ARROW)
        df = pd.DataFrame({
            'c1': [1, 2, 3],
            'c2': ['1', '2', '3'],
            'c3': [[1], [2], [3]]
        ...
        # TODO: toDF需要确认在arrow下如何修改，或文档注明该方法无效
        # print(conn1.run("t"))
        print(t.toDF())
        # assert t.equals()
    ...
    def test_upload_download_other_dataforms_with_enableArrow(self):
        ...
        assert conn1.run(
            """each(eqObj, [ar2,arV2,par2,matrx2],[ar,arV,par,matrx])""").any() == True
    ...
    def test_session_download_table_enableArrow_with_supportedTypes_gt_8192rows(self):
        ...
        table1 = conn1.run("""
                    ...
                    table1
                """)
        assert type(table1) == pyarrow.Table
        ...
        temp = pyarrow.table({
            ...
            'csymbol': pyarrow.chunked_array(pyarrow.DictionaryArray.from_arrays(pyarrow.array([1 for _ in range(10000)],
                                                                                               type=pyarrow.int32()), pyarrow.array(['', 'ddvb']))),
            ...
            'cuuid': pyarrow.array([b']!*x\xccH\xe3\xb1B5\xb4\xd9\x14s\xee\x87' for _ in range(10000)], type=pyarrow.binary(16)),
            'cint128': pyarrow.array([b'\xe1g\x17\x97\xc5.\x15\xf7c8\x0bE\xe8A\xec2' for _ in range(10000)], type=pyarrow.binary(16)),
        ...
        assert_array_almost_equal(table1['cfloat'].to_pylist(), [
                                  3.12437 for _ in range(10000)])
        assert_array_almost_equal(
            table1['cdouble'].to_pylist(), [-3.12437 for _ in range(10000)])
    ...
    def test_session_download_array_vector_table_enableArrow_with_supportedTypes_8192rows(self):
        ...
        temp = pyarrow.table({
            ...
            'cnanotimestamp': pyarrow.array([[0,0,0] for _ in range(10000)], type=pyarrow.list_(pyarrow.timestamp('ns'))),
            'cipaddr': pyarrow.array([["192.168.1.13","192.168.1.13","192.168.1.13"] for _ in range(10000)], type=pyarrow.list_(pyarrow.utf8())),
            'cuuid': pyarrow.array([[b']!*x\xccH\xe3\xb1B5\xb4\xd9\x14s\xee\x87',b']!*x\xccH\xe3\xb1B5\xb4\xd9\x14s\xee\x87',b']!*x\xccH\xe3\xb1B5\xb4\xd9\x14s\xee\x87'] for _ in range(10000)],
                                   type=pyarrow.list_(pyarrow.binary(16))),
            'cint128': pyarrow.array([[b'\xe1g\x17\x97\xc5.\x15\xf7c8\x0bE\xe8A\xec2',b'\xe1g\x17\x97\xc5.\x15\xf7c8\x0bE\xe8A\xec2',b'\xe1g\x17\x97\xc5.\x15\xf7c8\x0bE\xe8A\xec2'] for _ in range(10000)],
                                     type=pyarrow.list_(pyarrow.binary(16))),
        })
        assert table1['cnanotime'].equals(temp['cnanotime'])
        assert table1['cnanotimestamp'].equals(temp['cnanotimestamp'])
    ...
    def test_session_highAvailability_load_balance(self):
        ...
        for i in connList:
            time.sleep(1)
            i.connect(host=host,port=port0,userid="admin",password="123456",highAvailability=True,highAvailabilitySites=sites)
        loadDf=conn.run("select (connectionNum + workerNum + executorNum)/3.0 as load from rpc(getControllerAlias(), getClusterPerf) where mode=0")
    ...
    @pytest.mark.SESSION
    def test_session_python_parser(self):
        conn=ddb.Session(HOST,PORT,USER,PASSWD,python=True)
        conn.run("import dolphindb as ddb")
