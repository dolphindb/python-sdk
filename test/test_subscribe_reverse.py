...
class TestSubscribeReverse:
    ...
    def test_enableStreaming_subscribe_double_vector_msgAsTable_True(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(1)
        conn1.subscribe(HOST,PORT,self.handler_df(counter),"trades_doublev2","action",0,False, msgAsTable=True, batchSize=1000, throttle=1.0)
        assert counter.wait_s(20)
    ...
    def test_enableStreaming_subscribe_msgAsTable_false_all_datatype(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(10)
        conn1.subscribe(HOST, PORT, self.handler_batach_list(counter), "trades103", "action", 0, False, msgAsTable=False, batchSize=5)
        assert counter.wait_s(20)
        assert self.num==10, "1"
        df = pd.DataFrame(self.lst, columns=["boolv", "charv", "shortv", "intv", "longv", "datev", "monthv", "timev", "minutev", "secondv", "datetimev", "nanotimev", "timestampv", "nanotimestampv", "stringv", "doublev", "boolav", "charav", "shortav", "intav", "longav", "dateav", "monthav", "timeav", "minuteav", "secondav", "datetimeav", "nanotimeav", "timestampav", "nanotimestampav", "doubleav"])
        columns=["boolv", "charv", "shortv", "intv", "longv", "datev", "monthv", "timev", "minutev", "secondv", "datetimev", "nanotimev", "timestampv", "nanotimestampv", "stringv", "doublev", "boolav", "charav", "shortav", "intav", "longav", "dateav", "monthav", "timeav", "minuteav", "secondav", "datetimeav", "nanotimeav", "timestampav", "nanotimestampav", "doubleav"]
        for col in columns:
    ...
    def test_enableStreaming_subscribe_msgAsTable_false_all_datatype_one_row(self, enableStreaming):
        ...
        assert counter.wait_s(20)
        assert self.num==1, "1"
        ans = [True, 1, 1, 1, 1, np.datetime64('1970-01-02'), np.datetime64('2012-12'), np.datetime64('1970-01-01T00:00:00.001'), np.datetime64('1970-01-01T00:01'), np.datetime64('1970-01-01T00:00:01'), np.datetime64('1970-01-01T00:00:01'), np.datetime64('1970-01-01T00:00:00.000000001'), np.datetime64('1970-01-01T00:00:00.001'), np.datetime64('1970-01-01T00:00:00.000000001'), '1', 1.0, np.array([ True]), np.array([1, 2], dtype=np.int8), np.array([1], dtype=np.int16), np.array([1], dtype=np.int32), np.array([1]), np.array(['1970-01-02'], dtype='datetime64[D]'), np.array(['2012-12'], dtype='datetime64[M]'), np.array(['1970-01-01T00:00:00.001'], dtype='datetime64[ms]'), np.array(['1970-01-01T00:01'], dtype='datetime64[m]'), np.array(['1970-01-01T00:00:01'], dtype='datetime64[s]'), np.array(['1970-01-01T00:00:01'], dtype='datetime64[s]'), np.array(['1970-01-01T00:00:00.000000001'], dtype='datetime64[ns]'), np.array(['1970-01-01T00:00:00.001'], dtype='datetime64[ms]'), np.array(['1970-01-01T00:00:00.000000001'], dtype='datetime64[ns]'), np.array([1.])]
        columns=["boolv", "charv", "shortv", "intv", "longv", "datev", "monthv", "timev", "minutev", "secondv", "datetimev", "nanotimev", "timestampv", "nanotimestampv", "stringv", "doublev", "boolav", "charav", "shortav", "intav", "longav", "dateav", "monthav", "timeav", "minuteav", "secondav", "datetimeav", "nanotimeav", "timestampav", "nanotimestampav", "doubleav"]
        for i in range(len(ans)):
    ...
    def test_enableStreaming_subscribe_msgAsTable_false_rows_gt_65536_positive_number(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(70005)
        conn1.subscribe(HOST, PORT, self.handler_batach_list(counter), "trades103", "action", 0, False, msgAsTable=False, batchSize=10000)
        assert counter.wait_s(20)
    ...
    def test_enableStreaming_subscribe_msgAsTable_true_all_datatype(self,enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(10)
        conn1.subscribe(HOST, PORT, self.handler_batch_df(counter), "trades103", "action", 0, False, msgAsTable=True, batchSize=5)
        assert counter.wait_s(20)
        assert self.num==10, "1"
    ...
    def test_enableStreaming_subscribe_msgAsTable_true_rows_gt_65536_positive_number(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(70005)
        conn1.subscribe(HOST, PORT, self.handler_batch_df(counter), "trades103", "action", 0, False, msgAsTable=True, batchSize=10000)
        assert counter.wait_s(20)
        assert self.num==70005, "1"
        assert_frame_equal(self.df, conn1.run("select * from trades103"))
        conn1.unsubscribe(HOST, PORT, "trades103","action")
    ...
    def test_enableStreaming_subscribe_msgAsTable_true_cost_gt_create(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(2020)
        conn1.subscribe(HOST, PORT, self.handler_batch_df(counter), "trades103", "action", 0, False, msgAsTable=True, batchSize=1200, throttle=5)
        time.sleep(5.5)
        script="""
            for(i in 1..100){
                insert into trades103 values(take(now(), 20), take(`000905`600001`300201`000908`600002, 20), rand(1000,20)/10.0)
    ...
    def test_enableStreaming_subscribe_msgAsTable_true_cost_lt_create(self,enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(2020)
        conn1.subscribe(HOST, PORT, self.handler_batch_df_sleep_10(counter), "trades103", "action", 0, False, msgAsTable=True, batchSize=1200, throttle=2)
        time.sleep(1)
        script="""
            for(i in 1..100){
                insert into trades103 values(take(now(), 20), take(`000905`600001`300201`000908`600002, 20), rand(1000,20)/10.0)
    ...
    def test_enableStreaming_subscribe_msgAsTable_false_cost_gt_create(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(2020)
        conn1.subscribe(HOST, PORT, self.handler_batach_list(counter), "trades103", "action", 0, False, msgAsTable=False, batchSize=1200, throttle=5.5)
        time.sleep(10)
        script="""
            for(i in 1..100){
                insert into trades103 values(take(now(), 20), take(`000905`600001`300201`000908`600002, 20), rand(1000,20)/10.0)
    ...
    def test_enableStreaming_subscribe_msgAsTable_false_cost_lt_create(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(2020)
        conn1.subscribe(HOST, PORT, self.handler_batach_list_sleep_10(counter), "trades103", "action", 0, False, msgAsTable=False, batchSize=1200, throttle=5.5)
        script="""
            for(i in 1..100){
                insert into trades103 values(take(now(), 20), take(`000905`600001`300201`000908`600002, 20), rand(1000,20)/10.0)
            }
    ...
    def test_enableStreaming_subscribe_error_batchSize_string(self, enableStreaming):
        ...
        conn1.run(script)
        with pytest.raises(Exception):
            conn1.subscribe(HOST, PORT, self.handler, "trades15", "action", 0, False, np.array(["000905"]), False, "fdsf")
        conn1.close()
    ...
    def test_enableStreaming_subscribe_error_throttle_string(self, enableStreaming):
        ...
        conn1.run(script)
        with pytest.raises(Exception):
            conn1.subscribe(HOST, PORT, self.handler, "trades15", "action", 0, False, np.array(["000905"]), False, -1, "sdfse")
        conn1.close()
    ...
    @pytest.mark.SUBSCRIBE
    @pytest.mark.parametrize(argnames="enableStreaming", argvalues=[0], ids=["subscribe_reverse"])
    def test_enableStreaming_subscribe_double_arrayvector_msgAsTable_False(self, enableStreaming):
        conn1 = ddb.session()
        conn1.connect(HOST, PORT, "admin", "123456")
        conn1.enableStreaming(enableStreaming)
        df = pd.DataFrame(columns=["symbolv", "doublev"])
    ...
    @pytest.mark.SUBSCRIBE
    @pytest.mark.parametrize(argnames="enableStreaming", argvalues=[0], ids=["subscribe_reverse"])
    def test_enableStreaming_subscribe_double_arrayvector_msgAsTable_True(self, enableStreaming):
        conn1 = ddb.session()
        conn1.connect(HOST, PORT, "admin", "123456")
        conn1.enableStreaming(enableStreaming)
        self.df = pd.DataFrame(columns=["symbolv", "doublev"])
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(1)
        conn1.subscribe(HOST,PORT,self.handler_df(counter),"trades_doublev2","action",0,False, msgAsTable=True, batchSize=1000, throttle=1)
        assert counter.wait_s(20)
        assert_frame_equal(self.df, conn1.run("select * from trades_doublev2"))
        conn1.unsubscribe(HOST, PORT, "trades_doublev2","action")
    ...
    def test_enableStreaming_subscribe_batchSize_lt_zero(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(10)
        conn1.subscribe(HOST,PORT,gethandler(df, counter),"trades_doublev","action",0,False, msgAsTable=False, batchSize=-1)
        assert counter.wait_s(20)
        assert_frame_equal(df, conn1.run("select * from trades_doublev"))
        conn1.unsubscribe(HOST, PORT, "trades_doublev","action")
        conn1.close()
    ...
    def test_enableStreaming_subscribe_msgAsTable_False_batchSize_gt_zero(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(5)
        conn1.subscribe(HOST,PORT,gethandler_multi_row(df, counter),"trades_doublev","action",0,False, msgAsTable=False, batchSize=2)
        assert counter.wait_s(20)
        assert_frame_equal(df, conn1.run("select * from trades_doublev"))
        conn1.unsubscribe(HOST, PORT, "trades_doublev","action")
        conn1.close()
    ...
    def test_enableStreaming_throttle_gt_zero(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(1)
        conn1.subscribe(HOST,PORT,self.handler_df(counter),"trades14","action",0,False, msgAsTable=True, batchSize=1000, throttle=10.1)
    ...
    @pytest.mark.SUBSCRIBE
    @pytest.mark.parametrize(argnames="enableStreaming", argvalues=[0], ids=["subscribe_reverse"])
    def test_enalbeStreaming_streamDeserializer_error_msgAsTable_True(self, enableStreaming):
        conn1 = ddb.session()
        conn1.connect(HOST, PORT, "admin", "123456")
        conn1.enableStreaming(enableStreaming)
        ...
        sd=ddb.streamDeserializer({"msg1":"table1","msg2":"table2"}, conn1)
        with pytest.raises(Exception):
            conn1.subscribe(HOST,PORT, streamDSgethandler(df1, df2),"outTables7","action",0,False, msgAsTable=True, streamDeserializer=sd, batchSize=5)
        conn1.close()
    ...
    def test_enalbeStreaming_streamDeserializer_memory_talbe_session_None(self, enableStreaming):
        ...
        conn1.subscribe(HOST,PORT,streamDSgethandler(df1, df2, counter),"outTables6","action",0,False, msgAsTable=False, streamDeserializer=sd)
        assert counter.wait_s(20)
    ...
    def test_enalbeStreaming_streamDeserializer_PartitionedTable_table_session_None(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(20)
        sd=ddb.streamDeserializer({"msg1":['dfs://test_StreamDeserializer_pair', "pt1"],"msg2":['dfs://test_StreamDeserializer_pair',"pt2"]}, None)
        conn1.subscribe(HOST,PORT,streamDSgethandler(df1, df2, counter),"outTables5","action",0,False, msgAsTable=False, streamDeserializer=sd, userName="admin", password="123456")
        assert counter.wait_s(20)
    ...
    def test_enalbeStreaming_streamDeserializer_memory_table(self, enableStreaming):
        ...
        counter.reset(20)
        sd=ddb.streamDeserializer({"msg1":"table1","msg2":"table2"}, conn1)
        conn1.subscribe(HOST,PORT,streamDSgethandler(df1, df2, counter),"outTables4","action",0,False, msgAsTable=False, streamDeserializer=sd)
        assert counter.wait_s(20)
        assert_frame_equal(df1.loc[:, :"price2"], conn1.run("select * from table1"))
        assert_frame_equal(df2.loc[:, :"price1"], conn1.run("select * from table2"))
    ...
    def test_enalbeStreaming_streamDeserializer_memory_table_10000_rows(self, enableStreaming):
        ...
        counter.reset(10000)
        sd=ddb.streamDeserializer({"msg1":"table1","msg2":"table2"}, conn1)
        conn1.subscribe(HOST,PORT,streamDSgethandler(df1, df2, counter),"outTables3","action",0,False, msgAsTable=False, streamDeserializer=sd)
    ...
    def test_enalbeStreaming_streamDeserializer_PartitionedTable_table(self, enableStreaming):
        ...
        counter = CountBatchDownLatch(1)
        counter.reset(20)
        sd=ddb.streamDeserializer({"msg1":['dfs://test_StreamDeserializer_pair', "pt1"],"msg2":['dfs://test_StreamDeserializer_pair',"pt2"]}, conn1)
        conn1.subscribe(HOST,PORT,streamDSgethandler(df1, df2, counter),"outTables2","action",0,False, msgAsTable=False, streamDeserializer=sd, userName="admin", password="123456")
        assert counter.wait_s(20)
        assert_frame_equal(df1.loc[:, :"price2"], conn1.run("select * from pt1"))
        assert_frame_equal(df2.loc[:, :"price1"], conn1.run("select * from pt2"))
    ...
    def test_enalbeStreaming_streamDeserializer_PartitionedTable_table_10000_rows(self, enableStreaming):
        ...
        counter.reset(10000)
        sd=ddb.streamDeserializer({"msg1":['dfs://test_StreamDeserializer_pair', "pt1"],"msg2":['dfs://test_StreamDeserializer_pair',"pt2"]}, conn1)
        conn1.subscribe(HOST,PORT,streamDSgethandler(df1, df2, counter),"outTables1","action",0,False, msgAsTable=False, streamDeserializer=sd, userName="admin", password="123456")
        assert counter.wait_s(200)
        assert_frame_equal(df1.loc[:, :"price2"], conn1.run("select * from pt1"))
        assert_frame_equal(df2.loc[:, :"price1"], conn1.run("select * from pt2"))
    ...
    @pytest.mark.SUBSCRIBE
    @pytest.mark.parametrize(argnames="enableStreaming", argvalues=[None, "dsf", -1], ids=["port_none", "port_str", "port_error"])
    ...
    @pytest.mark.v130221
    @pytest.mark.parametrize(argnames="enableStreaming", argvalues=[0], ids=["subscribe_reverse"])
    def test_enalbeStreaming_gbk_encoding(self, enableStreaming):
        conn1 = ddb.session()
        conn1.connect(HOST, PORT, "admin", "123456")
        conn1.enableStreaming(enableStreaming)
        script="""
...
if __name__ == '__main__':
    pytest.main(["-s", "test/test_subscribe_reverse.py"])
